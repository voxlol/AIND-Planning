...
class AirCargoProblem(Problem):
    ...
    def result(self, state: str, action: Action):
        ...
        # For each old +Fluent, keep if its not removed
        for fluent in old_state.pos:
            if fluent not in action.effect_rem:
                new_state.pos.append(fluent)
        # For each fluent added by the action, add to newstate if its not already
        for fluent in action.effect_add:
            if fluent not in new_state.pos:
                new_state.pos.append(fluent)
        # For each -Fluent, keep if its not added by the action
        for fluent in old_state.neg:
            if fluent not in action.effect_add:
                new_state.neg.append(fluent)
        # For each fluent removed, add to newstate negative if its not already
