...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        states = self.s_levels[level] # returns a set of pgnode_s

        for state_node in states:
            for action in self.all_actions:
                if (state_node.is_pos and state_node.symbol in action.precond_pos) or (not state_node.is_pos and state_node.symbol in action.precond_neg):
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for neg_effect in node_a2.action.effect_rem:
            if neg_effect in node_a1.action.effect_add:
                return True

        for pos_effect in node_a2.action.effect_add:
            if pos_effect in node_a1.action.effect_rem:
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for actions_1 in node_s1.parents:
            for actions_2 in node_s2.parents:
                if not actions_1.is_mutex(actions_2):
                    return False
        return True
